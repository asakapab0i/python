...
class Field:
    ...
    def report_contents(self):
        ...
        for crop in self._crops:
            crop_report.append(crop.reports())
        for animal in self._animals:
            animal_report.append(animal.reports())
    ...
    def grow(self, light, food, water):
        ...
        if len(self._animals) > 0:
            #grow the animals (water available to all animals in the same )
            food_required = 0
            for animal in self._animals:
                needs = animal.needs()
            ...
            for animal in self._animals:
                ...
                if food >= needs["food need"]:
                    ...
                    #see if there is any additional
                    #food to give
                    if additional_food > 0:
                        additional_food -= 1
                        #add this to the feed to be given to
                        #the animal
                        feed += 1
                    #grow the animal
                    animal.grows(feed,water)
